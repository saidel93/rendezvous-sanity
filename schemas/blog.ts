...
export default defineType({
  ...
  fields: [
    ...
    defineField({ name: 'extrait', title: 'Extrait (résumé court)', type: 'text', rows: 2, group: 'base' }),
    defineField({ name: 'contenu', title: 'Contenu', type: 'array', group: 'base', of: [{ type: 'block' }] }),
    defineField({ name: 'imageUrl', title: 'Image (URL externe)', type: 'url', group: 'base' }),
    defineField({ name: 'image', title: 'Image (upload)', type: 'image', group: 'base', options: { hotspot: true } }),
    defineField({ name: 'datePublication', title: 'Date de publication', type: 'date', group: 'base' }),
